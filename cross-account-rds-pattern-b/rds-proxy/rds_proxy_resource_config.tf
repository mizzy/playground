...
resource "aws_vpclattice_resource_configuration" "rds_proxy" {
  name                        = "pattern-b-rds-proxy-config"
  type                        = "SINGLE"
...
resource "aws_vpclattice_resource_configuration" "rds_proxy_reader" {
  ...
  tags = {
    Name = "pattern-b-rds-proxy-reader-resource-config"
  }
}

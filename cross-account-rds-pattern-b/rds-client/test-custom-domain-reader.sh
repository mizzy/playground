...
ORIGINAL_DNS_READER="pattern-b-rds-proxy-reader.endpoint.proxy-cpo0q8m8sxzx.ap-northeast-1.rds.amazonaws.com"
...
echo "=== Pattern B Custom Domain Name Test - RDS Proxy Reader ==="
echo ""
...
TASK_ARN=$(aws-vault exec rds-client -- aws ecs run-task \
  ...
  --overrides "{\"containerOverrides\":[{\"name\":\"postgres-client\",\"command\":[\"/bin/sh\",\"-c\",\"echo 'DNS Resolution Test:' && getent ahosts ${ORIGINAL_DNS_READER} && echo '' && echo 'Connection Test:' && PGPASSWORD=password123 psql -h ${ORIGINAL_DNS_READER} -U postgres -d testdb -c 'SELECT current_user, inet_server_addr(), pg_is_in_recovery();' && sleep 30\"]}]}" \

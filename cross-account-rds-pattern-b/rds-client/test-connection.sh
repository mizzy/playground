...
echo "=== Pattern B RDS Proxy Connection Test ==="
echo "開始時刻: $(date)"
echo ""
...
case "${ENDPOINT_TYPE}" in
  proxy-writer)
    ENDPOINT="pattern-b-rds-proxy.proxy-cpo0q8m8sxzx.ap-northeast-1.rds.amazonaws.com"
    ENDPOINT_NAME="RDS Proxy Writer"
    ;;
  proxy-reader)
    ENDPOINT="pattern-b-rds-proxy-reader.endpoint.proxy-cpo0q8m8sxzx.ap-northeast-1.rds.amazonaws.com"
    ENDPOINT_NAME="RDS Proxy Reader"
    ;;
  aurora-writer)
    ENDPOINT="pattern-b-aurora-cluster.cluster-cpo0q8m8sxzx.ap-northeast-1.rds.amazonaws.com"
    ENDPOINT_NAME="Aurora Cluster Writer"
    ;;
  aurora-reader)
    ENDPOINT="pattern-b-aurora-cluster.cluster-ro-cpo0q8m8sxzx.ap-northeast-1.rds.amazonaws.com"
...
SUBNET_IDS=$(aws-vault exec rds-client -- aws ec2 describe-subnets \
  --filters "Name=tag:Name,Values=pattern-b-private-*" \
  --query 'Subnets[].SubnetId' --output text | tr '\t' ',')

SG_ID=$(aws-vault exec rds-client -- aws ec2 describe-security-groups \
  --filters "Name=tag:Name,Values=pattern-b-ecs-tasks-sg" \
